...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DragScrollDirective, MaterialsModule],
...
export class BoardCanvasComponent implements OnInit, OnDestroy {
  ...
  // 사이드 바 썸네일 윈도우 크기
  containerSize: WritableSignal<ContainerSize> =
    this.canvasService.containerSize;
  ...
  ngOnInit(): void {
    this.initCanvasSet();
    this.rendererEvent = this.renderer.listen(
  ...
  initCanvasSet() {
    ...
    CANVAS_CONFIG.maxContainerHeight =
      window.innerHeight -
      CANVAS_CONFIG.toolbarHeight -
      CANVAS_CONFIG.navHeight; // pdf 불러오기 사이즈
  ...
  pageRender(currentPage: number, zoomScale: number) {
    ...
    // PDF Rendering
    this.renderingService.renderBackground(
      this.tmpCanvas,
      this.bgCanvas,
      currentPage
    );
  ...
  onResize() {
    console.log('resize!!');
    if (!this.pdfInfo().pdfDocument)
      return console.log(this.pdfInfo().pdfDocument);
  ...
  onChangePage() {
    console.log(
      `>> changePage to page: ${this.currentPage()}, scale: ${this.zoomScale()} `
    );
