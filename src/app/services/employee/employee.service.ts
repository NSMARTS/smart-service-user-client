...
export class EmployeeService {
  ...
  employeeList(sort: string, order: SortDirection, page: number, pageSize: number, username: string) {
    return this.http.get(this.baseUrl + `/manager/employee?sort=${sort}&order=${order}&page=${page + 1}&pageSize=${pageSize}&username=${username}`);
  }
